...
api_key = "ZpJ7czdNzkkAq2HHg9KqCbwYjThPKDwjs7ywnRtpv1fxDsRs68ftVd48YwsATBad"
secret_key = "fA5TTjO3eGzHWDfipFF1kQC6WkIDPqa3l7N0hN004gkOZzUgO10iLZrWPvyDhchD"
    ...
    for number_coin in range(2000):
        ...
        if number_coin == 0:
            ...
            for i in range(1):
                ...
                import yfinance as yf
                real_date = str(time.strftime("%Y-%m-%d"))
                dataLTCBTC = yf.download("LTC-BTC", start="2022-09-01", end=real_date, interval="1h")
                dataLTCBTC_fc = dataLTCBTC.filter(["Close"])
                dataLTCBTC_fc.to_csv("LTCBTC_1h.csv")

                print(dataLTCBTC_fc)
                plt.plot(dataLTCBTC_fc)
                plt.savefig('data_LTCBTC_filter_close_1m.jpg')
                ...
                data_safe_file_LTCBTC.write(",")
                data_safe_file_LTCBTC.write(str(balance_LTC["free"]))
                data_safe_file_LTCBTC.write(",")
                data_safe_file_LTCBTC.write(str(balance_btc["free"]))
                data_safe_file_LTCBTC.write(",")
                data_safe_file_LTCBTC.write("\n")
                data_safe_file_LTCBTC.close()

                data_read_pandas_LTCBTC = pd.read_csv("LTCBTC.csv")
                #data_read_pandas_LTCBTC = data_read_pandas_LTCBTC.tail(500)
                data_read_pandas_LTCBTC_shape_row,data_read_pandas_LTCBTC_shape_col = data_read_pandas_LTCBTC.shape[0],data_read_pandas_LTCBTC.shape[1]
                print(data_read_pandas_LTCBTC.shape)
                print([data_read_pandas_LTCBTC_shape_row,data_read_pandas_LTCBTC_shape_col])

                filter_LTCBTC_price = data_read_pandas_LTCBTC.filter(["price_LTCBTC"])

                print(filter_LTCBTC_price)

                # create dATEFRAME CLOSE
                data = data_read_pandas_LTCBTC.filter(["price_LTCBTC"])
                ...
                new_df = data_read_pandas_LTCBTC.filter(["price_LTCBTC"])
                ...
                if preset_pred_price <= float(symbol_LTCBTC_price):
                    ...
                    if data_coin <= 0:
                        ...
                        quantity_start = round(quantity, 4)
                        print(quantity_start)
                        order = client.order_limit_buy(symbol='LTCBTC',quantity=quantity_start,price=preset_pred_price)
                        print(order)
                        data_safe_file_BTCRUB = open("LTCBTCorder.csv", "a")
                        data_safe_file_BTCRUB.write(str(time.strftime("%Y-%m-%d %H:%M:%S+00:00")))
                        data_safe_file_BTCRUB.write(",")
                        data_safe_file_BTCRUB.write(str(symbol_ETHBTC_price))
                        data_safe_file_BTCRUB.write(",")
                    ...
                    if data_coin <= 0:
                        ...
                        quantity_start = round(quantity, 4)
                        print(quantity_start)
                        order = client.order_limit_sell(symbol='LTCBTC',quantity=quantity_start,price=preset_pred_price)
                        print(order)
                        data_safe_file_ETHBTC = open("LTCBTCorder.csv", "a")
                        data_safe_file_ETHBTC.write(str(time.strftime("%Y-%m-%d %H:%M:%S+00:00")))
                        data_safe_file_ETHBTC.write(",")
                        data_safe_file_ETHBTC.write(str(symbol_LTCBTC_price))
                        data_safe_file_ETHBTC.write(",")
                        data_safe_file_ETHBTC.write(str(order))
                        data_safe_file_ETHBTC.write(",")
                        data_safe_file_ETHBTC.write("\n")
