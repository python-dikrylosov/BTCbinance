...
while True:
    ...
    for i in range(0):
        ...
        symbol_BTCRUB_symbol = symbol_BTCRUB["symbol"]
        symbol_BTCRUB_price = symbol_BTCRUB["price"]
        data_safe_file_BTCRUB = open("BTCRUB.csv","a")
        data_safe_file_BTCRUB.write(str(time.strftime("%Y-%m-%d %H:%M:%S+00:00")))
        data_safe_file_BTCRUB.write(",")
        data_safe_file_BTCRUB.write(str(symbol_BTCRUB_price))
        data_safe_file_BTCRUB.write(",")
        data_safe_file_BTCRUB.write(str(balance_RUB["free"]))
        data_safe_file_BTCRUB.write(",")
        data_safe_file_BTCRUB.write(str(balance_btc["free"]))
        data_safe_file_BTCRUB.write(",")
        data_safe_file_BTCRUB.write("\n")
        data_safe_file_BTCRUB.close()

        data_read_pandas_BTCRUB = pd.read_csv("BTCRUB.csv")
        data_read_pandas_BTCRUB = data_read_pandas_BTCRUB.tail(500)
        data_read_pandas_BTCRUB_shape_row,data_read_pandas_BTCRUB_shape_col = data_read_pandas_BTCRUB.shape[0],data_read_pandas_BTCRUB.shape[1]
        print(data_read_pandas_BTCRUB.shape)
        print([data_read_pandas_BTCRUB_shape_row,data_read_pandas_BTCRUB_shape_col])

        filter_BTCRUB_price = data_read_pandas_BTCRUB.filter(["price_BTCRUB"])

        print(filter_BTCRUB_price)

        # create dATEFRAME CLOSE
        data = data_read_pandas_BTCRUB.filter(["price_BTCRUB"])
        ...
        model = tf.keras.models.load_model(os.path.join("./dnn/", "BTC-RUB_model.h5"))
        model.fit(x_train, y_train, batch_size=1, epochs=1)

        model.save(os.path.join("./dnn/", "BTC-RUB_model.h5"))
        #reconstructed_model = tf.keras.models.load_model(os.path.join("./dnn/", "BTC-RUB_model.h5"))
        ...
        new_df = data_read_pandas_BTCRUB.filter(["price_BTCRUB"])
        ...
        if preset_pred_price <= float(symbol_BTCRUB_price):
            ...
            if data_coin <= 0:
                ...
                print(quantity)
                print("Недостаточно  RUB")
            elif data_coin >= 0:
                print([data_coin, a, b])
                print("\n" + "SELL Покупать RUB  " + str(preset_pred_price))
                print(a)
                quantity = float(min_lovume_rub / float(symbol_BTCRUB_price))
                quantity_start = round(quantity, 5)
                print(quantity_start)
                order = client.order_limit_buy(symbol='BTCRUB',quantity=0.0012,price=preset_pred_price)
                print(order)
                data_safe_file_BTCRUB = open("BTCRUBorder.csv", "a")
                data_safe_file_BTCRUB.write(str(time.strftime("%Y-%m-%d %H:%M:%S+00:00")))
                data_safe_file_BTCRUB.write(",")
                data_safe_file_BTCRUB.write(str(symbol_BTCRUB_price))
                data_safe_file_BTCRUB.write(",")
            ...
            if data_coin <= 0:
                ...
                quantity_start = round(quantity, 5)
                print(quantity_start)
                order = client.order_limit_sell(symbol='BTCRUB',quantity=0.0012,price=preset_pred_price)
                print(order)
                data_safe_file_BTCRUB = open("BTCRUBorder.csv", "a")
                data_safe_file_BTCRUB.write(str(time.strftime("%Y-%m-%d %H:%M:%S+00:00")))
                data_safe_file_BTCRUB.write(",")
